Вот полный script.js под схему, где:

обычные файлы открываются в браузере

office-файлы (doc/docx/xls/xlsx/ppt/pptx/...) открываются через штатные страницы Bitrix Disk

редактирование office-файлов идет через editUrl

viewer внутри кастомной страницы больше не требуется

папки, загрузка, создание, удаление, переименование, поиск, breadcrumbs — все остается
...
(function () {
  ...
  DiskComponent.prototype.bindStaticEvents = function () {
    ...
    this.root.addEventListener('click', async function (e) {
      ...
      if (clickableRow && !e.target.closest('button, input, label, a, span')) {
