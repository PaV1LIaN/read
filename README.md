Вот final public.php с вложенным left menu.

Что изменено:

leftMode = menu теперь строит дерево опубликованных страниц по parentId

активная страница подсвечивается

активная ветка раскрывается

дочерние страницы показываются с отступом

left/right/header/footer и текущая раскладка сохранены
...
function sb_render_left_menu_tree(array $nodes, array $site, int $currentPageId, array $activePathIds, int $level = 0): string {
    ...
    foreach ($nodes as $node) {
        ...
        $html .= '<div class="sideTreeNode level-' . $level . '">';
        $html .= '<a class="' . h(implode(' ', $classes)) . '" href="' . h($url) . '">';
        if ($hasChildren) {
            $html .= '<span class="sideMenuCaret">' . ($isInPath ? '▾' : '▸') . '</span>';
        } else {
            $html .= '<span class="sideMenuCaret sideMenuCaretEmpty"></span>';
        }
        $html .= '<span class="sideMenuText">' . h($title) . '</span>';
        $html .= '</a>';

        if ($hasChildren && $isInPath) {
            $html .= '<div class="sideTreeChildren">';
...
1. у одной страницы поставь parentId = 0


2. у нескольких других поставь её как родителя


3. включи showLeft


4. поставь leftMode = menu


5. открой дочернюю страницу
...
родительская ветка раскроется

текущая страница подсветится.