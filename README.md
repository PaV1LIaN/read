Вот полный script.js, где:

папки открываются по клику

обычные файлы (pdf/png/jpg/txt/...) открываются в браузере

office-файлы (docx/xlsx/pptx/...) открываются через штатный data-viewer Bitrix, как в твоем рабочем примере

есть отдельная кнопка Скачать

breadcrumbs укорочены

работает bootstrap, upload, createFolder, rename, delete, search, settings
...
После замены обязательно сделай Ctrl + F5.

Если после этого docx все равно не откроется, пришли HTML одной кнопки Открыть для docx из инспектора браузера.