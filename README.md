Ниже полностью рабочий script.js с исправленным sessid fallback и запуском через bootstrap.
...
(function () {
  ...
  DiskComponent.prototype.setViewMode = function (mode) {
    ...
    if (gridContainer) {
      ...
      if (this.state.viewMode === 'grid' && !gridContainer.classList.contains('sb-disk__grid')) {
...
После замены обязательно сделай Ctrl + F5, чтобы браузер не взял старую версию файла из кэша.