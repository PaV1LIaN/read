Ок, я “причесал” твой `editor.php`:

* убрал дубли имён (`ec`),
* собрал хелперы вместе,
* все `add*/edit*` функции сгруппированы,
* общий click-handler аккуратный и без конфликтов,
* добавил недостающие `.catch()` там, где их не было (чтобы не было “тишины” при ошибке),
* сохранил весь твой функционал: **Text / Image / Button / Heading / Columns2 / Gallery / Spacer / Card**.

Заменяй файл **полностью**: `/local/sitebuilder/editor.php`
...
### Что именно было “не так” в твоём файле

Главное: **двойной `const ec` в одном обработчике клика** — это могло полностью ломать JS (и тогда “ничего не работает” без понятной причины). Здесь это исправлено.

Если этот файл встал — говори “готово”, и двинемся дальше: **следующий блок я бы сделал “Cards grid”** (сетку из карточек 2/3/4 в ряд), это уже уровень “готовые секции” как в тильде.
