Вот готовый `public.php` целиком:
...
function public_page_url(int $siteId, ?array $page): string {
    ...
    if ($slug !== '') {
        return '/local/sitebuilder/public.php?siteId=' . $siteId . '&p=' . urlencode($slug);
    }
...
<div class="wrap">
  ...
  <div class="meta">
    ...
    <div>siteId: <code><?= (int)$siteId ?></code> • pageId: <code><?= (int)$pageId ?></code></div>
  </div>
</div>
...
* если страница `draft`, она не должна открываться по `public.php?...&p=slug`
* если в меню есть draft-страница, ссылка на неё не должна выводиться
* если `homePageId` указывает на draft, должна открыться первая опубликованная страница
* если опубликованных страниц нет, будет `Page not found`

Следующим шагом я бы сделал красивый `404` и редирект с `public.php?siteId=...` на человекочитаемый URL по slug.
